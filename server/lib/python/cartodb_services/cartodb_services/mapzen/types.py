...
def polyline_to_linestring(polyline):
    ...
    try:
        ...
        cartodb_services.plpy.warning("Can't generate LINESTRING from polyline: {0}".format(e))
        geometry = None
...
def country_to_iso3(country):
    ...
    try:
        country_plan = cartodb_services.plpy.prepare("SELECT adm0_a3 as iso3 FROM admin0_synonyms WHERE lower(regexp_replace($1, " \
                                    "'[^a-zA-Z\u00C0-\u00ff]+', '', 'g'))::text = name_; ", ['text'])
        country_result = cartodb_services.plpy.execute(country_plan, [country], 1)
        if country_result:
            return country_result[0]['iso3']
        else:
            return None
    except BaseException as e:
        cartodb_services.plpy.warning("Can't get the iso3 code from {0}: {1}".format(country, e))
        return None
