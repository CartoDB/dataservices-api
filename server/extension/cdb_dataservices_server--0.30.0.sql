...
RETURNS cdb_dataservices_server.simple_route AS $$
  ...
  with metrics('cdb_route_with_point', user_routing_config, logger=logger):
...
RETURNS cdb_dataservices_server.simple_route AS $$
  ...
  with metrics('cdb_route_with_waypoints', user_routing_config, logger=logger):
    mapzen_plan = plpy.prepare("SELECT * FROM cdb_dataservices_server._cdb_mapzen_route_with_waypoints($1, $2, $3, $4, $5, $6) as route;", ["text", "text", "geometry(Point, 4326)[]", "text", "text[]", "text"])
    result = plpy.execute(mapzen_plan, [username, orgname, waypoints, mode, options, units])
    return [result[0]['shape'],result[0]['length'], result[0]['duration']]
$$ LANGUAGE plpythonu STABLE PARALLEL RESTRICTED;
...
RETURNS json AS $$
  ...
  with metrics('obs_getdemographicsnapshot', user_obs_config, logger=logger):
    try:
...
RETURNS json AS $$
  ...
  with metrics('obs_getsegmentsnapshot', user_obs_config, logger=logger):
    try:
...
RETURNS NUMERIC AS $$
  ...
  with metrics('obs_getmeasure', user_obs_config, logger=logger):
    try:
...
RETURNS TEXT AS $$
  ...
  with metrics('obs_getcategory', user_obs_config, logger=logger):
    try:
...
RETURNS NUMERIC AS $$
  ...
  with metrics('obs_getuscensusmeasure', user_obs_config, logger=logger):
    try:
...
RETURNS TEXT AS $$
  ...
  with metrics('obs_getuscensuscategory', user_obs_config, logger=logger):
    try:
...
RETURNS NUMERIC AS $$
  ...
  with metrics('obs_getpopulation', user_obs_config, logger=logger):
    try:
...
RETURNS NUMERIC AS $$
  ...
  with metrics('obs_getmeasurebyid', user_obs_config, logger=logger):
    try:
...
RETURNS TABLE (
  ...
  with metrics('obs_getdata', user_obs_config, params, logger=logger):
    try:
...
RETURNS JSON AS $$
  ...
  with metrics('obs_getmeta', user_obs_config, params, logger=logger):
    try:
...
RETURNS TABLE(valid boolean, errors text[]) AS $$
  ...
  with metrics('obs_metadatavalidation', user_obs_config, logger=logger):
    try:
...
RETURNS TABLE(id text, description text, name text, aggregate text, source text) AS $$
  ...
  with metrics('obs_search', user_obs_config, logger=logger):
    try:
...
RETURNS TABLE(boundary_id text, description text, time_span text, tablename text) AS $$
  ...
  with metrics('obs_getavailableboundaries', user_obs_config, logger=logger):
    try:
...
RETURNS geometry(Geometry, 4326) AS $$
  ...
  with metrics('obs_getboundary', user_obs_config, logger=logger):
    try:
...
RETURNS TEXT AS $$
  ...
  with metrics('obs_getboundaryid', user_obs_config, logger=logger):
    try:
...
RETURNS geometry(Geometry, 4326) AS $$
  ...
  with metrics('obs_getboundarybyid', user_obs_config, logger=logger):
    try:
...
RETURNS TABLE(the_geom geometry, geom_refs text) AS $$
  ...
  with metrics('obs_getboundariesbygeometry', user_obs_config, logger=logger):
    try:
...
RETURNS TABLE(the_geom geometry, geom_refs text) AS $$
  ...
  with metrics('obs_getboundariesbypointandradius', user_obs_config, logger=logger):
    try:
...
RETURNS TABLE(the_geom geometry, geom_refs text) AS $$
  ...
  with metrics('obs_getpointsbygeometry', user_obs_config, logger=logger):
    try:
...
RETURNS TABLE(the_geom geometry, geom_refs text) AS $$
  ...
  with metrics('obs_getpointsbypointandradius', user_obs_config, logger=logger):
    try:
...
RETURNS Geometry AS $$
  ...
  with metrics('cdb_geocode_street_point', user_geocoder_config, logger=logger):
...
RETURNS Geometry AS $$
  ...
  with metrics('cdb_geocode_admin0_polygon', user_geocoder_config, logger=logger):
    try:
...
RETURNS Geometry AS $$
  ...
  with metrics('cdb_geocode_admin1_polygon', user_geocoder_config, logger=logger):
    try:
...
RETURNS Geometry AS $$
  ...
  with metrics('cdb_geocode_namedplace_point', user_geocoder_config, logger=logger):
    try:
...
RETURNS Geometry AS $$
  ...
  logger = Logger(logger_config)
  quota_service = QuotaService(user_geocoder_config, redis_conn)
  with metrics('cdb_geocode_postalcode_point', user_geocoder_config, logger=logger):
    try:
...
RETURNS Geometry AS $$
  ...
  logger = Logger(logger_config)
  quota_service = QuotaService(user_geocoder_config, redis_conn)
  with metrics('cdb_geocode_ipaddress_point', user_geocoder_config, logger=logger):
    try:
...
RETURNS SETOF cdb_dataservices_server.isoline AS $$
  ...
  with metrics('cb_isodistance', user_isolines_config, logger=logger):
...
RETURNS SETOF cdb_dataservices_server.isoline AS $$
  ...
  with metrics('cb_isochrone', user_isolines_config, logger=logger):
    if user_isolines_config.heremaps_provider:
