...
CREATE OR REPLACE FUNCTION cdb_dataservices_client._DST_PopulateTable_DEBUG(
    table_name text,
...
BEGIN
  ...
  SELECT cdb_dataservices_client.__DST_PopulateTable_DEBUG(
      username,
      orgname,
      user_db_role,
      user_schema,
...
CREATE OR REPLACE FUNCTION cdb_dataservices_client.__DST_PopulateTable_DEBUG(
    username text,
    orgname text,
    user_db_role text,
    user_schema text,
    ...
    if ds_return_metadata[0]["colnames"]:
        ...
        raise Exception('Error retrieving DEBUG metadata')
